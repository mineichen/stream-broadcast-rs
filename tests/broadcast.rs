...
mod tests {
    ...
    use futures::{future::join, stream::FusedStream as _, FutureExt as _, Stream, StreamExt as _};
    use stream_broadcast::{StreamBroadcastExt as _, StreamBroadcastLossy};
    ...
    #[tokio::test]
    async fn new_broadcast_ignores_previous() -> TestResult {
        let stream = futures::stream::iter(0..3).fuse();
        let mut broadcast = StreamBroadcastLossy::new(stream, 3);
        broadcast.next().await.ok_or("Should be here")?;
        let broadcast2 = broadcast.clone();
    ...
    async fn parallel() -> TestResult {
        ...
        let (r1, r2) = join(task::spawn(stream1.count()), task::spawn(stream2.count())).await;
        assert_eq!(r1?, ITERATIONS);
        assert_eq!(r2?, ITERATIONS);
        Ok(())
    }
    ...
    async fn two_lossless_clones_see_all_items() {
        ...
        let (all_a, all_b) = join(a.collect::<Vec<_>>(), b.collect::<Vec<_>>()).await;
    ...
    async fn lossless_and_lossy_share_the_buffer() {
        ...
        let stream = futures::stream::iter(0..10).fuse();
        let lossless = stream.broadcast_lossless(2);
        let lossy = lossless.create_lossy().collect::<Vec<_>>();

        let (all_lossless, all_lossy) = join(lossless.collect::<Vec<_>>(), lossy).await;
    ...
    #[tokio::test]
    async fn dropping_lagging_lossless_unblocks_others() -> TestResult {
        let stream = futures::stream::iter(0..10).fuse();
        let lossless = stream.broadcast_lossless(2);
        let mut lossy = lossless.create_lossy();
        ...
        // dropping the lagging lossless subscriber should unblock the lossy one
        let rest = timeout_fast(lossy.collect::<Vec<_>>()).await?;
    ...
    #[tokio::test]
    async fn create_lossy_from_lossless_does_not_backpressure() -> TestResult {
        let stream = futures::stream::iter(0..10).fuse();
        let lossless = stream.broadcast_lossless(2);
        ...
        let all = timeout_fast(lossy.collect::<Vec<_>>()).await?;
        assert_eq!(10, all.len());
        Ok(())
    }

    #[tokio::test]
    async fn lossless_drains_buffer_after_input_terminated() -> TestResult {
        ...
        let stream = futures::stream::iter(0..2).fuse();
        let lossless = stream.broadcast_lossless(2);
        let lossy = lossless.create_lossy().count();
        let lossless = lossless.collect::<Vec<_>>();
        let (lossy_count, remaining) = timeout_fast(join(lossy, lossless)).await?;
    ...
    #[tokio::test]
    async fn weak_create_lossy_and_lossless() -> TestResult {
        let stream = futures::stream::iter(0..5).fuse();
        let lossless = stream.broadcast_lossless(5);
        let weak = lossless.create_lossy().downgrade();

        let mut upgraded_lossy = weak.create_lossy().ok_or("broadcast is still alive")?;
        assert_eq!(Some((0, 0)), upgraded_lossy.next().await);

        let mut upgraded_lossless = weak.create_lossless().ok_or("broadcast is still alive")?;
    ...
    #[tokio::test]
    async fn parallel_lossless() -> TestResult {
        const ITERATIONS: usize = 50;
        let stream1 = futures::stream::iter(0..ITERATIONS)
            .fuse()
        ...
        let count1 = stream1
            .then(move |x| {
                ...
                tokio::time::sleep(Duration::from_micros(seed1.into())).map(move |()| x)
            })
            .count();
        let count2 = stream2
            .then(move |x| {
                seed2 = seed2.wrapping_mul(43);
                tokio::time::sleep(Duration::from_micros(seed2.into())).map(move |()| x)
            })
            .count();

        let (r1, r2) = join(task::spawn(count1), task::spawn(count2)).await;
        assert_eq!(r1?, ITERATIONS);
        assert_eq!(r2?, ITERATIONS);
        Ok(())
    }
    ...
    #[expect(
        deprecated,
        reason = "proves the deprecated API still compiles and works"
    )]
